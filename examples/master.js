...
function sendMasterMessage(message) {
    Object.keys(master.dataChannelByClientId).forEach(clientId => {
        try {
            ...
            if(message.action == "GET_PLAYLIST"){
                var json_payload_master={
                    "sessionId":message.sessionId,
                    "action": "PLAYLIST",
                    "payload": { 
                        "streamType":"PLAYBACK",
                        "url" : "http://localhost:5000/master.m3u8"
                    }
                }
            }

            else if(message.action == "GET_FILE"){
                var json_payload_master = {
                    "sessionId":message.sessionId,
                    "action": "LAST_CHUNK",
            ...
            master.dataChannelByClientId[clientId].send(JSON.stringify(json_payload_master));
        } catch (e) {
